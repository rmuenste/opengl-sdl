  ...
  class ShadowMapTest : public SDL_GL_application
  {
    public:
      ...
      void init()
      {
        ...
        perspective_.setPerspectiveTransform(50.0, getWindowWidth(), getWindowHeight(), 3.0, 9.0);
        ...
        light_ = Light(Vec3(0, 1, -6.0), Vec3(1.0, 1.0, 1.0), 0.5f);
        ...
        roomTextureShader_.initShader(perspective_.getPerspectiveTransform(), shadowCamera_.getCameraTranslationTransform(), shadowCamera_.getCameraCoordinateTransform());
        ...
        twoPassShadow_.initShader(light_.getPos(), perspective_.getPerspectiveTransform(), camera_.getCameraTranslationTransform(), camera_.getCameraCoordinateTransform());
        ...
        //for (MeshIter mesh = quad_.model_.begin(); mesh != quad_.model_.end(); mesh++)
        //{
        //  for (MeshVertexIter i = mesh->meshVertexBegin(); i != mesh->meshVertexEnd(); i++)
        //  {
        //    VECTOR3 x = *i;
        //    Vec4 v(x.x, x.y, x.z,1.0);
        //    Vec4 ndc = (perspective_.getPerspectiveTransform() * camera_.getCameraTranslationTransform() * camera_.getCameraCoordinateTransform() * quad_.transform_.getMatrix()) * v;
        //    float d = ndc.w;
        //    std::cout << ndc * (1.0f/d) << std::endl;
        //  }
        //}
        //
        //exit(0);
        ...
        worldTextureShader_.initShader(perspective_.getPerspectiveTransform(), shadowCamera_.getCameraTranslationTransform(), shadowCamera_.getCameraCoordinateTransform());
      ...
      void render()
      {
        ...
        biasMatrix.m_00 = 0.5f;
        biasMatrix.m_11 = 0.5f;
        biasMatrix.m_22 = 0.5f;
        biasMatrix.m_03 = 0.5f;
        biasMatrix.m_13 = 0.5f;
        biasMatrix.m_23 = 0.5f;
        biasMatrix.m_33 = 1.0f;

        Mat4 shadow = perspective_.getPerspectiveTransform() * shadowCamera_.getCameraTranslationTransform() * shadowCamera_.getCameraCoordinateTransform();
        twoPassShadow_.bind();
        
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, depthTexture_.id_);
        GLint uniform_loc1 = glGetUniformLocation(twoPassShadow_.program_, "depthTexture");
        glUniform1i(uniform_loc1, 1);

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, room_.texture_.id_);
        GLint uniform_loc0 = glGetUniformLocation(twoPassShadow_.program_, "sampler");
        glUniform1i(uniform_loc0, 0);

        twoPassShadow_.setShadowMatrix(shadow);
        room_.shader_ = &twoPassShadow_;
        room_.shader_->material_ = &roomMat_;
        room_.render();

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, world_.texture_.id_);
        uniform_loc0 = glGetUniformLocation(twoPassShadow_.program_, "sampler");
        glUniform1i(uniform_loc0, 0);
        world_.shader_ = &twoPassShadow_;
        world_.shader_->material_ = &worldMat_;
        world_.render();
