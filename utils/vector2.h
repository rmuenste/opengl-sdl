...
/**
* @brief A class for a 2d vector and various 2d vector operations
*
* A class for a 2d vector and various 2d vector operations
*/  
template<class T>
class CVector2 {
...
public:
	...
	friend std::ostream& operator<<(std::ostream& out, const CVector2<Templateparm>& v1); 
	...
	inline double CalcAngle(CVector2 v1)
	{
		...
		nAng = nAng * 180.0/double(CMath<T>::SYS_PI);
...
template<class T> CVector2<T> operator*(T a,const CVector2<T> &vRHS);
...
{
	// Return scaled vector
	return CVector2<T>(vRHS.x * a, vRHS.y * a);
}//end  operator


template<class T>
std::ostream& operator<<(std::ostream& out, const CVector2<T> &v1) 
{
	return out << "("<<v1.x<<","<<v1.y<<")"<<std::endl;
}
